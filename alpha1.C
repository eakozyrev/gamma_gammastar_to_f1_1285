...
double prob_ma0(double *s, double *par)
{
  ...
  {
    norm = 1.;
    N = tree0->GetEntries(cut);
  }
  if (par[1] == 1)
  {
    norm = tree1->GetEntries((TCut)SELECT.c_str());
    N = tree1->GetEntries(cut);
  }
  if (par[1] == 2)
  {
    norm = tree2->GetEntries((TCut)SELECT.c_str());
    N = tree2->GetEntries(cut);
  }
  if (par[1] == 3)
  {
    norm = tree3->GetEntries((TCut)SELECT.c_str());
    N = tree3->GetEntries(cut);
  }
  if (par[1] == 4)
  {
    norm = tree4->GetEntries((TCut)SELECT.c_str());
    N = tree4->GetEntries(cut);
  }
  if (par[1] == 5)
  {
    norm = 1.;
    N = 0.5 * tree0->GetEntries(Form((SELECT + " && ifsig==0 && ma0p > %g && ma0p < %g && ma0m > %g && ma0m < %g").c_str(), mpl - wide, mpl + wide, mmi - wide, mmi + wide));
  }
  // cout << N << " " << norm << endl;
  return par[0] * N / norm;
}
...
{
  if (mode == 0)
    return (double)tree_m0_gen->GetEntries(Form("Q2gen > %g && Q2gen < %g", par0, par1)) / tree_m0_gen->GetEntries() * cross_total_m0;
  else if (mode == 1)
    return (double)tree_m1_gen->GetEntries(Form("Q2gen > %g && Q2gen < %g", par0, par1)) / tree_m1_gen->GetEntries() * cross_total_m1;
  else
    return 0.;
}
...
double prob_cosa0(double *s, double *par)
{
  ...
  if (par[1] == 1)
  {
    norm = tree1->GetEntries((TCut)(SELECT + "&& ifsig==1").c_str());
    N = tree1->GetEntries(cut) * (1. - alphamc / 2.);
  }
  if (par[1] == 2)
  {
    norm = tree2->GetEntries((TCut)(SELECT + "&& ifsig==1").c_str());
    N = tree2->GetEntries(cut) * (1. - alphamc / 2.);
  }
  if (par[1] == 3)
  {
    norm = tree3->GetEntries((TCut)(SELECT + "&& ifsig==1").c_str());
    N = tree3->GetEntries(cut) * (1. - alphamc / 2.);
  }
  if (par[1] == 4)
  {
    norm = tree4->GetEntries((TCut)(SELECT + "&& ifsig==1").c_str());
    N = tree4->GetEntries(cut) * (1. - alphamc / 2.);
  }
  if (par[1] == 5)
  {
    norm = 1.;
...
void compare_cos(string filee, string filemc0, string filemc1, double factorm1, string variable, string cutt = "1==1")
{
  ...
  hm2ph_mc->Draw("same");
  hm2ph_b->SetNormFactor(0.5 * hm2ph_b->GetEntries());
  hm2ph_b->Draw("same");
  hm2ph_b->SetFillStyle(3001);
  hm2ph_b->SetLineColor(13);
  hm2ph_b->SetFillColor(13);

  hm2ph_mc1->SetLineColor(1);
  hm2ph_mc1->SetLineWidth(3);
  hm2ph_mc1->Draw("same");
  hm2ph_mc0->SetLineColor(2);
  hm2ph_mc0->SetLineWidth(3);
  hm2ph_mc0->Draw("same");
  hm2ph->Draw("esame");
...
void compare(string filee, string filemc0, string filemc1, double factorm0, string variable, string cutt = "1==1", string xtitle = "")
{
  ...
  TH1D *h1;
  TH1D *hm2ph = new TH1D("hm2ph", "", 40, start, end);
  ...
  TH1D *hMC = new TH1D("hMC", "hMC", 40, start, end);
  hMC->Add(hm2ph_mc0, (double)notm * factorm0 / hm2ph_mc0->GetEntries());
  hMC->Add(hm2ph_mc1, (double)notm * (1. - factorm0) / hm2ph_mc1->GetEntries());
...
void compare_cos_all(string filee, string filemc0, string filemc1, double factorm0, string variable, string cutt = "1==1", string name_fs = "")
{
  ...
  hm2ph_mc->SetFillStyle(3002);
  hm2ph_mc->Draw("same");
  hm2ph_b->SetNormFactor(factor * hm2ph_b->GetEntries());
  hm2ph_b->Draw("same");
  hm2ph_b->SetFillStyle(3001);
  hm2ph_b->SetLineColor(13);
  hm2ph_b->SetFillColor(13);
...
void cross_section()
{
  ...
  while (stream.eof() == 0)
  {
    ...
    double Ngen_m0 = (double)treem0gen->GetEntries(Form("Q2gen > %g && Q2gen < %g", ena, enb));
    double Ngen_m1 = (double)treem1gen->GetEntries(Form("Q2gen > %g && Q2gen < %g", ena, enb));
    double eff_m0 = effic_Q2(ena, enb, 0, false, nselected0, ntotal0);
    double deff_m0 = sqrt(eff_m0 * (1. - eff_m0) / Ngen_m0);
    double eff_m1 = effic_Q2(ena, enb, 1, false, nselected1, ntotal1);
    double deff_m1 = sqrt(eff_m1 * (1. - eff_m1) / Ngen_m1);
    ...
    double deff_m0_true = sqrt(eff_m0_true * (1. - eff_m0_true) / Ngen_m0);
    double deff_m1_true = sqrt(eff_m1_true * (1. - eff_m1_true) / Ngen_m1);

    auto eff_true = (nselected0 + nselected1) / (ntotal0 + ntotal1);
    auto deff_true = sqrt(eff_true * (1. - eff_true) / (Ngen_m0 + Ngen_m1));
    ...
    auto roundFF = [](double val) {
      return double((int)(val * 10000.)) / 10000;
    };

    auto roundNev = [](double val) {
      return double((int)(val * 100.)) / 100;
    };

    stream_table_cr << ena << "$\\div$" << enb << " & " << roundNev(Nev) << " $\\pm$ " << roundNev(dNev) << " & "
                    ...
                    << " & " << roundNev(cross_m1 + cross_m0) << " $\\pm$ " << roundNev(sqrt(dcross_m1 * dcross_m1 + dcross_m0 * dcross_m0))
                    << " & " << roundFF(ff_total*ff0) << " $\\pm$ " << roundFF(dff_total*ff0) << " \\\\" << endl;
  }
}
...
void draw_cross_sect(int mode = 0)
{
  ...
  double *unc_cor_cr1 = new double[nrun];
  for (int i = 0; i < nrun; i++) {
    unc_cor_cr1[i] = dcr2[i] * total[i];
  }
  ...
  double *unc_cor_cr0 = new double[nrun];
  for (int i = 0; i < nrun; i++) {
    unc_cor_cr0[i] = dcr2[i] * cr0[i];
  }
...
double effic_Q2(double left, double right, int m, bool FF, double &nselected,
                double &ntotal)
{
  ...
  for (int i = 0; i < Nsteps; i++)
  {
    ...
    // m2pigen < 0.55 &&
    double Neff = tree_effic->GetEntries(
        Form("mf1 > 1.17 && mf1 < 1.4 && Q2 > %g && Q2 < %g", q2 - step / 2.,
             q2 + step / 2.));
    if (FF)
    {
    ...
    res += factor * Neff;
    res0 += factor *
            (double)tree_effic_gen->GetEntries(Form(
                "Q2gen > %g && Q2gen < %g", q2 - step / 2., q2 + step / 2.));
  }
  nselected = res;
  ntotal = res0;
  return res / res0;
